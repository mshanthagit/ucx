...
//#include <hip/hip_runtime.h>         // why is this required?

#define UCT_ROCM_IPC_IS_ALIGNED_POW2(_n, _p) (!((_n) & ((_p) - 1)))
#define UCT_ROCM_IPC_WAVEFRONT_SIZE 64
...
__device__ static inline void* uct_rocm_ipc_map_remote(
    const uct_rocm_ipc_device_mem_element_t* elem,
    uint64_t remote_address)
{
    return reinterpret_cast<void*>(
        (uintptr_t)remote_address + elem->mapped_offset);
}

/* System-wide atomic increment */
__device__ static inline void uct_rocm_ipc_atomic_inc(uint64_t *dst, uint64_t inc_value)
{
    atomicAdd_system((unsigned long long*)dst, 
                     (unsigned long long)inc_value);
...
/* Thread-level copy */
template<>
__device__ inline void uct_rocm_ipc_copy_level<UCS_DEVICE_LEVEL_THREAD>(
...
template<>
__device__ inline void uct_rocm_ipc_copy_level<UCS_DEVICE_LEVEL_WARP>(
    void *dst, const void *src, size_t len)
{
    using vec4 = int4;
    using vec2 = int2;
...
/* Block-level copy - optimized for AMD architecture */
template<>
__device__ inline void uct_rocm_ipc_copy_level<UCS_DEVICE_LEVEL_BLOCK>(
    void *dst, const void *src, size_t len)
{
    using vec4 = int4;
    using vec2 = int2;
...
template<>
__device__ inline void uct_rocm_ipc_copy_level<UCS_DEVICE_LEVEL_GRID>(
    void *dst, const void *src, size_t len)
{
    /* Not implemented */
}

/* Single PUT operation - renamed for consistency with CUDA IPC */
template<ucs_device_level_t level = UCS_DEVICE_LEVEL_BLOCK>
__device__ ucs_status_t
uct_rocm_ipc_ep_put(uct_device_ep_h device_ep,
                    ...
                    uct_device_completion_t *comp)
{
    auto rocm_ipc_mem_element =
        reinterpret_cast<const uct_rocm_ipc_device_mem_element_t *>(mem_elem);
...
template<ucs_device_level_t level = UCS_DEVICE_LEVEL_BLOCK>
__device__ ucs_status_t
uct_rocm_ipc_ep_atomic_add(uct_device_ep_h device_ep,
                           const uct_device_mem_element_t *mem_elem,
...
/* Get local pointer to remote memory - for feature parity with CUDA IPC */
